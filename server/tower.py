...
from utils import read_json, decrypt_battle_data
...
def towerDropSystem(stageCnt: int, player_data: dict, drop: list) -> None:
    ...
    if player_data["tower"]["current"]["status"]["isHard"] == True:
...
def towerRecord(player_data: dict) -> None:
    ...
    elif player_data["tower"]["current"]["status"]["isHard"] == True:
...
def towerBattleStart() -> Response:
    ...
    player_data["status"]["battleId"] = str(base64.b64encode(request_data["stageId"].encode('utf-8')), 'utf-8')
...
def towerBattleFinish() -> Response:
    ...
    player_data = json.loads(accounts.get_user())
    TOWER_TABLE = updateData(TOWER_TABLE_URL, True)
    BattleData = BattleData = decrypt_battle_data(request_data["data"], player_data["pushFlags"]["status"])
